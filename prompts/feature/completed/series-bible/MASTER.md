# Feature Build — Master Loop (series-bible)
...
**Name:** series-bible
**Intent:** Add series support — group multiple books into ordered series with a shared story bible that persists across volumes and is automatically loaded into agent context.
**Total sessions:** 7
...
You are building the "series-bible" feature for Novel Engine. This is a set of 7 ordered session prompts that implement the feature from domain types through to the renderer UI.
...
1. **Read state.** Read `prompts/feature/series-bible/STATE.md` to see what's done, what's pending, and any handoff notes from previous runs.
...
2. **Pick the next session.** Select the first `pending` session whose dependencies are satisfied.
   ...
   - SESSION-04: SESSION-01, SESSION-02, SESSION-03
   - SESSION-05: SESSION-04
   - SESSION-06: SESSION-05
   - SESSION-07: SESSION-05, SESSION-06

3. **Read the session prompt.** Read `prompts/feature/series-bible/SESSION-NN.md` in full.
...
7. **Update state.** Edit `prompts/feature/series-bible/STATE.md`:
...
9. **Commit.** Stage and commit the session's changes:
   ```bash
   git add -A && git commit -m "feat(series-bible): SESSION-NN — {short title}"
   ```
...
**Phase 1 — Backend (Sessions 1-4)**
Sequential: Domain types -> Infrastructure -> Application -> IPC wiring

**Phase 2 — Frontend (Sessions 5-7)**
Sequential: Store -> Sidebar groups -> Management modal
