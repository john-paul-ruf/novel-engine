# SESSION-04 — Renderer: FindReplaceModal and FilesView Integration
...
SESSION-03 wired `FindReplaceService` into the IPC layer and exposed it via `window.novelEngine.findReplace`. The service is fully operational.

This session adds the UI:
1. `FindReplaceModal.tsx` — the full find/replace dialog (search, preview, apply, summary)
2. A "Find & Replace" button in `FilesHeader.tsx`
3. Modal state wiring in `FilesView.tsx`

No new Zustand store is needed — all modal state is local (it's a single-shot operation with no persistence requirement).
...
|------|--------|-------------|
| `src/renderer/components/Files/FindReplaceModal.tsx` | Create | Full find/replace modal component |
| `src/renderer/components/Files/FilesHeader.tsx` | Modify | Add `onFindReplace?: () => void` prop and button |
| `src/renderer/components/Files/FilesView.tsx` | Modify | Add `showFindReplace` state; mount modal; pass prop to header |
...
Read all three target files in full before writing any code:
- `src/renderer/components/Files/FilesHeader.tsx` — understand the existing `FilesHeaderProps` type and the right-side button area
- `src/renderer/components/Files/FilesView.tsx` — understand the modal pattern (the `DeleteConfirmModal` is already mounted here; follow the same pattern)

Also check `src/renderer/components/Files/DeleteConfirmModal.tsx` to understand how modals are currently constructed in this component family.

### 2. Create `src/renderer/components/Files/FindReplaceModal.tsx`

This is a modal overlay component with three phases: **input**, **preview**, and **result**.

```typescript
import { useState, useCallback } from 'react';
import { useBookStore } from '../../stores/bookStore';
import type { FindReplaceApplyResult, FindReplaceOptions, FindReplacePreviewItem, FindReplacePreviewResult } from '@domain/types';
```
...
**`toggleExpand` (expand/collapse match locations):**
...
**JSX structure** — the modal is a fixed overlay:
...
**`InputSection` sub-component** (defined in the same file, not exported):

Renders two stacked text inputs (Search / Replacement) and two toggle buttons (Case Sensitive, Regex). Use a grid layout. The replacement field is optional — if empty, the apply call will delete all occurrences. Include a small note: "Leave replacement empty to delete matches."
...
**`ToggleButton` sub-component** (defined in the same file, not exported):
...
**`PreviewSection` sub-component** (defined in the same file, not exported):

Renders the match count summary, select-all/deselect-all buttons, and the chapter table. Each row shows:
- Checkbox
- Chapter filename (the slug extracted from `filePath`, e.g. `01-the-beginning`)
- Match count badge
- Expand/collapse chevron

When expanded, renders up to 20 match locations. Each location shows the `lineNumber` and `lineText` with the matched portion highlighted (bold + colour).

The match highlight renders the lineText sliced into three parts: before, match, after.
...
**`ChapterMatchRow` sub-component** (defined in the same file, not exported):
...
**`MatchLine` sub-component** (defined in the same file, not exported):
...
**`ResultSection` sub-component** (defined in the same file, not exported):
...
### 3. Modify `src/renderer/components/Files/FilesHeader.tsx`

Read the file first. Add `onFindReplace?: () => void` to the `FilesHeaderProps` type, and add it as a prop in the destructured parameter.

Add the Find & Replace button to the right-side button area, **before** the view mode switcher group. It should always be visible (not dependent on `filePath` or `viewMode`):
...
### 4. Modify `src/renderer/components/Files/FilesView.tsx`

Read the file first. Make three additions:

**Step A — Import `FindReplaceModal`:**

```typescript
import { FindReplaceModal } from './FindReplaceModal';
```

**Step B — Add modal state** (alongside the existing `showHistory` / `showDeleteConfirm` state):

```typescript
const [showFindReplace, setShowFindReplace] = useState(false);
```

**Step C — Wire the header and mount the modal.** Pass `onFindReplace={() => setShowFindReplace(true)}` to `<FilesHeader />`, and mount the modal conditionally right before the closing `</div>` of the root container:
...
- [x] Renderer accesses backend only through `window.novelEngine.findReplace.*`
- [x] `FindReplaceModal` uses `import type` for domain types — no value imports from domain
- [x] No `any` types
- [x] All async operations (preview, apply) have try/catch with error display
- [x] `useEffect` cleanup — not needed here (no subscriptions or timers)
- [x] No prop-drilling beyond 2 levels (modal manages its own state)
...
2. Launch the app, open the Files view — "⇄ Find & Replace" button appears in the header
3. Click the button — the modal opens with two inputs and toggle buttons
4. Type a search term and click Preview — results appear with chapter rows and match counts
5. Expand a chapter row — individual match locations are shown with the matched text highlighted
6. Deselect some chapters, click Replace — only the selected chapters are modified
7. After replace, the success banner shows the correct replacement count
8. Open a modified chapter in version history — a 'user' snapshot exists from before the replace
9. Close the modal — the Files view is unchanged
...
- Mark the feature as complete if all sessions are done
