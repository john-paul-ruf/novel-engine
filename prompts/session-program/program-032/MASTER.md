# Forge Build — Novel Engine / fix-phantom-turns-renderer-reads

## Protocol — Each iteration:

1. Read `FORGE-CONFIG.md` (registry, stack, conventions, verification)
2. Read `prompts/session-program/program-032/STATE.md` (done, pending, blocked)
3. Pick next pending session whose dependencies are all done
4. Read `prompts/session-program/program-032/SESSION-NN.md` fully + Module Context files
...
8. Update `prompts/session-program/program-032/STATE.md` (status, date, notes, handoff)
9. Update architecture if new module or changed public API
10. Commit (format: `feat(fix-phantom-turns-renderer-reads): SESSION-NN — {title}`)
