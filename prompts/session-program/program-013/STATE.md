...
### SESSION-06 (2026-07-08) — Architecture + Changelog Pages

- **Both pages pre-existed from v0.7.0 — updated, not rebuilt.** No "two-call Wrangler" phantom claim existed on the old architecture page (good).
- **changelog.html:** appended the **14 missing entries** as full `<details>` blocks generated mechanically from `CHANGELOG.md` (12× 2026-07-02 provider work: Codex ×8 incl. add-dir/workspace hardening, Ollama CLI-first ×3, 250K-context compaction ×1; plus 2026-03-28 Deployment Prep and 2026-03-29 Small-queue intake). Stats updated: **78 entries**, range 2026-03-27 — 2026-07-02, 187 source files. 3 new highlight-reel cards (Codex provider, CLI-first Ollama, 250K compaction). **Count nuance for SESSION-08:** page has 76 entry blocks for 78 `##` source entries — the v0.7.0 page merged two multi-session programs into single blocks; fuzzy content-coverage check confirms all 78 entries represented (only borderline: "Onboarding SESSION-04 Tooltips" lives inside the merged 5-session block).
