...
| 02 | Route Ollama availability and model discovery through CLI | M06, M09 | pending |  | Make startup and provider checks CLI-first, with API fallback only for configured remote endpoints and chat protocol needs. |
...
- Next session can wire `OllamaCliRunner` into startup/provider availability without changing domain contracts.
